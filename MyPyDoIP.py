...
#for s brand, all IP address of edge node set as:
defaultTargetIPAddr = '169.254.19.1'
...
class DoIP_Client:
    ...
    def __init__(self, address='0', port=13400):
        ...
        self._localPort = port  #port=13400，TCP 通讯时，两端都使用这个端口号
        self._localECUAddr = '8E01'
        self._targetIPAddr = '169.254.19.1'
        self._targetPort = 13400
        self._targetECUAddr = '1a01'
        ...
        if self._isTCPConnected:
            ...
            if self._TCP_Socket != None:
                try:
                    ...
                    self._targetIPAddr = address    #'169.254.19.1'
                    self._targetPort = port # 13400
                    self._TCP_Socket.connect((address, port))
                    #should have a judgement about the connection situation
    ...
    def RequestRoutingActivation(self, activationType=DEFAULT_ACTIVATION,\
            localECUAddr='169.254.102.85', targetECUAddr=''):
        if self._isTCPConnected:
            try:
                if not localECUAddr:
...
#	testMsg = DoIPMsg(DOIP_PV+DOIP_IPV+DOIP_UDS+plLen+udspl+srcAddr+trgtAddr+'5001',verbose = True)
    ...
    # DoIPClient.ConnectToDoIPServer()
    time.sleep(5)
    DoIPClient.ConnectToDoIPServer(defaultTargetIPAddr, 13400, True, '1A01')
    time.sleep(5)
